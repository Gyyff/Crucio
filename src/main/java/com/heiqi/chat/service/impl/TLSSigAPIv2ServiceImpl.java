...
import com.heiqi.chat.Utils.Base64ConvertUtil;
 ...
 * @ClassName
 * @Author bonies
 * @Date 2023年12月04日 10:42
 * @Description
 */
@Service
public class TLSSigAPIv2ServiceImpl implements TLSSigAPIv2Service {
   ...
   *
   * @param userid - User ID. The value can be up to 32 bytes in length and contain letters (a-z and
   *     A-Z), digits (0-9), underscores (_), and hyphens (-).
  ...
  /**
   * 【功能说明】 用于签发 TRTC 进房参数中可选的 PrivateMapKey 权限票据。 PrivateMapKey 需要跟 UserSig 一起使用，但 PrivateMapKey 比
   * UserSig 有更强的权限控制能力： - UserSig 只能控制某个 UserID 有无使用 TRTC 服务的权限，只要 UserSig 正确，其对应的 UserID 可以进出任意房间。
   * - PrivateMapKey 则是将 UserID 的权限控制的更加严格，包括能不能进入某个房间，能不能在该房间里上行音视频等等。 如果要开启 PrivateMapKey
   ...
   * @param privilegeMap - 权限位，使用了一个字节中的 8 个比特位，分别代表八个具体的功能权限开关： - 第 1 位：0000 0001 = 1，创建房间的权限 - 第 2
   *     位：0000 0010 = 2，加入房间的权限 - 第 3 位：0000 0100 = 4，发送语音的权限 - 第 4 位：0000 1000 = 8，接收语音的权限 - 第 5
   *     位：0001 0000 = 16，发送视频的权限 - 第 6 位：0010 0000 = 32，接收视频的权限 - 第 7 位：0100 0000 =
   ...
   * can be used to enter a specific room and perform audio/video upstreaming in the room. To enable
   * stringent PrivateMapKey permission bit verification, you need to enable permission key in TRTC
   ...
   *     specified by roomid. - privilegeMap == 0010 1010 == 42: Indicates that the UserID has only
   *     the permissions to enter the room and receive audio/video data.
   ...
   * @return usersig - Generate signature with userbuf
   */
  public String genPrivateMapKeyWithStringRoomID(
  ...
  private String genUserSig(String userid, long expire, byte[] userbuf) {
    ...
    compressor.end();
    return (new String(
            Base64ConvertUtil.base64EncodeUrl(
  ...
  public byte[] genUserBuf(
    ...
    */
    ...
    // The fields required for the video check digit are placed in buf according to the network byte
    // order.
    /*
